...
type userR struct {
	UserGameSessions UserGameSessionSlice `boil:"UserGameSessions" json:"UserGameSessions" toml:"UserGameSessions" yaml:"UserGameSessions"`
}
...
func (r *userR) GetUserGameSessions() UserGameSessionSlice {
	if r == nil {
		return nil
	}
	return r.UserGameSessions
}
...
func (o *User) UserGameSessions(mods ...qm.QueryMod) userGameSessionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_game_sessions\".\"user_id\"=?", o.ID),
	)

	return UserGameSessions(queryMods...)
}

// LoadUserGameSessions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserGameSessions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_game_sessions.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load user_game_sessions")
	}

	var resultSlice []*UserGameSession
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice user_game_sessions")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on user_game_sessions")
	}
	...
	if singular {
		object.R.UserGameSessions = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.UserGameSessions = append(local.R.UserGameSessions, foreign)
				if foreign.R == nil {
					foreign.R = &userGameSessionR{}
				}
...
// AddUserGameSessions adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UserGameSessions.
...
func (o *User) AddUserGameSessions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserGameSession) error {
	...
	if o.R == nil {
		o.R = &userR{
			UserGameSessions: related,
		}
	} else {
		o.R.UserGameSessions = append(o.R.UserGameSessions, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &userGameSessionR{
				User: o,
			}
		} else {
			rel.R.User = o
